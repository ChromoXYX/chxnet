...
struct ssl_no_ktls : SSLOperation {
    struct im_read {};
    struct final_write {};
    int current_ec = 0;

    template <typename... Args>
    ssl_no_ktls(Stream* s, Args&&... args) noexcept(
    ...
    template <typename Cntl> void perform(Cntl& cntl) {
        ...
        if (r == 1) {
            ...
            if (current_ssl_ec == SSL_ERROR_WANT_READ) {
                ...
                cntl.complete(make_ssl_ec(current_ssl_ec));
            }
        }
    }
    ...
    void operator()(Cntl& cntl, const std::error_code& e, std::size_t s) {
        if (current_ec) {
            if (cntl.tracked_task_empty()) {
                cntl.complete(net::detail::make_ec(current_ec));
            }
        } else if (e) {
            current_ec = e.value();
            if (cntl.tracked_task_empty()) {
